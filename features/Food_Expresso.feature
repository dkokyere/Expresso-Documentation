Feature: Expresso food Feature
    ...
    Scenario: Members on the session wants to initiate a food order
        ...
        Then wait for Responses from members

    Scenario: Member has received notification for food initiation.
        Given Member has receive notification for "food order"
        Then Show notification to either "Accept Offer" or "Reject Offer"
        Given Member has "Accepted offer"
        ...
        Given Member has "Rejected offer"
        Then return to "Current Page" member was on
    ...
    Scenario: Member wants to make payment
        ...
        Then display "order tracking page"

    Scenario: User want to see order status in progress
        Given User has place an "order"
        Then display "summary of order"
        And display "status of order"